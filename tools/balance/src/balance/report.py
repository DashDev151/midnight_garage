...
INVARIANTS_ENFORCED_SECTION = [
    ...
    "standard test suite). 9 more are measured and reported but NOT gated - see "
    "`invariants.py`'s module docstring for their full history. Sprint 79 (maintainer "
    "sign-off 2026-07-16) demoted 5 of those 9 (days-to-`local`, the buyout-share "
    "ceiling, and the 3 legacy Sprint 03/09 checks): all five read bot-career CSVs "
    "(`careers.csv`, `auctionWins.csv`, `acquisitions.csv`), and the bots do not "
    "faithfully simulate the post-arc game (no inspection, no teardown, no "
    "build-to-spec) - a bot-derived pass or fail is not design evidence either way "
    "(`TODO.md`'s bot-harness entry). The remaining 4 were already informational before "
    "Sprint 79 and read BADLY, deliberately: as of Sprint 66 most strategies lose money "
    "(Flipper is well below its own starting cash) and the auction tail is "
    "frenzy-dominant. Do not tune the economy against any of these 9 figures. They "
    "measure BOT behaviour, and the bots restore every car to mint - which "
    "economy-bible law 1, as amended in Sprint 66, now correctly punishes on a cheap "
