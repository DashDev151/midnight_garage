...
describe('saveCodec', () => {
   ...
   * `CarInstance` gained `acquisitionEvidenceDelta`, `.optional()` - a
   * pre-v76 car simply reads it absent, which `priorBand` already treats as
   * 0 (no adjustment), so no dedicated pre-version decode test is needed
   * beyond the general `.optional()` coverage every field of that shape
   * already gets. This is the round-trip half: a v76 state with a car that
   * has the frozen delta carries it through exactly.
   */
  it('round-trips a real car with a frozen acquisitionEvidenceDelta', () => {
    const withEvidence: GameState = GameStateSchema.parse({
      ...fullState,
      ownedCars: [
        {
          id: 'car-0001',
          ...
          provenanceNote: '',
          parts: mintParts(),
          verifiedSlots: ['tyres', 'rims'],
          acquisitionEvidenceDelta: -1,
        },
      ],
    })
    const decoded = decodeSave(encodeSave(withEvidence))
    expect(decoded).toEqual(withEvidence)
  })
  ...
  it('a v35 state with real offered and active mission records round-trips storyMissions exactly', () => {
    const withMissions: GameState = GameStateSchema.parse({
      ...fullState,
      storyMissions: [
        { missionId: 'placeholder-a', status: 'offered', acceptedOnDay: null },
        { missionId: 'placeholder-b', status: 'active', acceptedOnDay: 10 },
      ],
    })
  ...
  /**
   * v45 -> v46 (auction room admission): `GameState` gained an optional
   * `attendanceFeePaidDayByTier`. The pure additive case (the genuinely-
   * optional-key pattern): every other test in this file already proves the
   * absent case round-trips (none of them set this field), so this is the
   * one test for the present case.
   */
  it('a v46 state round-trips attendanceFeePaidDayByTier exactly', () => {
    const withAttendance: GameState = GameStateSchema.parse({
      ...fullState,
      attendanceFeePaidDayByTier: { regional: 12, premium: 12 },
    })
  ...
  /**
   * v63 -> v64 (word of mouth and scene commissions): `GameState` gained an
   * optional `sceneCommissions`. The pure additive case (the genuinely-
   * optional-key pattern): every other test in this file already proves the
   * absent case round-trips (none of them set this field), so this is the
   * one test for the present case.
   */
  it('a v64 state round-trips sceneCommissions exactly', () => {
    const withCommissions: GameState = GameStateSchema.parse({
      ...fullState,
      sceneCommissions: {
        collector: null,
        tuner: {
          customerName: 'Gen',
          requestCopy: 'Wants the numbers, not the story.',
          requirements: [{ kind: 'statThreshold', stat: 'handling', min: 55 }],
          status: 'offered',
          postedOnDay: 12,
          acceptedOnDay: null,
        },
