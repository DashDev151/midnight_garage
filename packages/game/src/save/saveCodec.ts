 ...
 *
 * - v1: first save.
 * - v2: added `reputationPoints`, `serviceJobOffers`,
 ...
 *   still loads).
 * - v3: added `serviceBayCount`, `parkingBayCount`,
 ...
 * - v4: added `laborSlotsSpentToday` to GameState (the live daily
 ...
 * - v5: `CarInstance`'s `condition`/`buildSheet` split collapsed
 *   into one unified `components` map (zones+slots -> components refactor).
 *   Deliberately **no `MIGRATIONS[4]` step** - there are no existing saves
 *   worth preserving, so a pre-v5 save's `CarInstance`
 ...
 * - v6: added `ownedEquipmentIds` to GameState (what REPAIR is
 ...
 * - v7: added `pendingPartOrders` and `cartPartIds` to GameState
 ...
 *   step needed.
 * - v8: `PublicListing` (nested in `activeListings`) gained
 ...
 * - v9: `serviceBayCarIds` changed shape from a compact list of
 ...
 *   old list (the same exclusion rule the old `parkingView`
 ...
 *   visible).
 * - v10: added `stagedCarWork` - per-car repair/install work the
 ...
 * - v11: `AuctionLot` (nested in `activeAuctionLots`) gained
 ...
 * - v12 (auction rework II): `AuctionLot` swaps its whole bid-state
 ...
 * - v13 (value model): added `marketLedger` to GameState - the
 ...
 * - v14 (hidden issues): every `CarInstance.hiddenIssues[]` entry
 ...
 * - v15: `ServiceJob` gained `arrivesOnDay` - accepting a
 ...
 *   step needed.
 * - v16 (the banded parts model): the single biggest structural
 ...
 *   following the locked group-to-parts mapping: each old group's
 ...
 *   old-model counterpart at all and always migrates to `mint` (an explicit
 *   carve-out); `forcedInduction.fitted` is `true` only on a
 ...
 *   entry above), but this migration calls for tag- and
 ...
 *   (the old behaviour every repair implicitly had); every
 ...
 * - v17 (per-part addressing): `JobSchema` and
 ...
 *   bump is what makes an older client REJECT a newer save
 ...
 *   degrade per-part staged work back to group-level. `saveCodec.test.ts`
 *   covers this with three tests (immediately before the v15->v16 migration
 *   `describe` block): a v16 group-level save still decodes
 ...
 *   `carPartId` exactly.
 * - v18 (service-jobs framework v2): `ServiceJobSchema`'s single
 ...
 *     group>` targeting `mint` (every repair before v16 implicitly
 ...
 *     reshaped by `migrateServiceJobToTasks` itself into the current
 ...
 *     `payoutYen` and `dueOnDay` are left untouched (never re-derive a live
 *     job's economics) - `deadlineDays`
 ...
 *     7-day historical fallback (the old constant's own value)
 *     when `arrivesOnDay` isn't available to reconstruct it from.
 *   - `serviceJobOffers` (not yet accepted): DROPPED, not mapped. Offers
 *     refresh daily
 ...
 * - v19 (living auctions): `AuctionLot` gained `turnout` - the
 *   rival-turnout band, rolled once at lot creation and
 *   persisted (replacing the old always-recomputed `turnoutBand` function,
 *   deleted this version). Purely additive with a schema default of
 ...
 * - v20 (the walk-in offer stream): `PublicListing`/
 ...
 *   (a for-sale car now draws a live, same-day-only offer instead).
 ...
 * - v21 (stock-baseline/missing-slot model): `CarPartState`
 ...
 *   `migrateV20ToV21` reconstructs it per a locked
 ...
 *   has carried its own `band` since v16 and needs no reconstruction;
 ...
 *   part is `grade: 'stock'` for that `CarPartId` (exactly one is
 *   guaranteed to exist). No pre-v21 save has a "missing" slot (the
 ...
 * - v22 (customer-owned parts + in-inventory reconditioning): two
 ...
 *   migration or not) - it's the guard that makes an older client
 *   REJECT a newer save (`decodeSave`'s `envelope.version > SAVE_VERSION`
 *   throws "newer version") rather than silently strip `customerJobId` and
 *   quietly unlock a customer's part for sale. `saveCodec.test.ts` covers it
 *   with two tests: a real v21 save with an untagged inventory part
 ...
 * - v23 (tool lines): `ownedEquipmentIds` (binary equipment
 *   ownership, added at v6) is replaced by `toolTiers` - six always-owned tool
 ...
 *   no-ceiling floor, which is exactly this version's authored content too.
 * - v24 (specialty - the progression bible's horizontal axis):
 ...
 * - v25 (the flip ledger): `GameStateSchema` gained `carLedgers`
 ...
 * - v26 (the double-parking grace slot): `GameStateSchema` gained
 ...
 *   concept did not exist - every acquisition before this either placed
 ...
 * - v27 (the used-machinery classifieds): `GameStateSchema` gained
 ...
 * - v28 (fitment-class parts): the catalog gained real per-class
 *   SKUs (economy-bible.md law 3) - the original 116 ids are kept
 ...
 *   host car) and `pendingPartOrders` are left as `common` (the
 ...
 */
/**
 * v28 -> v29 (the job ledger): `GameStateSchema` gained
 ...
 * v29 -> v30 (baseline-tracked installs): `ServiceJobSchema` gained
 ...
 * v30 -> v31 (parts provenance): `PartInstanceSchema` gained a
 ...
 * v31 -> v32 (outcome-based service jobs): `ServiceJobTaskSchema`
 ...
 * v32 -> v33 (diagnosis I): `CarInstanceSchema` gained two new
 ...
 * v34 -> v35 (story missions I): `GameState` gained `storyMissions`
 ...
 * version bump alone is still required (Save law).
 * v35 -> v36 (the equivalence-priced labour model): each car
 ...
 * v36 -> v37 (staff I): `GameState` gained `staffAds` (default
 ...
 * v37 -> v38 (crew-model rework): `StaffMemberSchema` changed shape -
 ...
 * v38 -> v39 (story missions unfailable):
 ...
 * v39 save. (The single bump for this version - the rest add only content
 * knobs and transactional fees, no new persisted GameState shape.)
 * v39 -> v40 (the assembly model): `GameStateSchema` gained
 * `assemblyInventory` - sub-assemblies currently on the bench
 * (`AssemblyContainerSchema`). It is `.optional()` (not `.default([])`), the
 * same genuinely-optional treatment v36's `vacatedBaseline` used, so a
 ...
 * than silently dropping the field. (The single bump for this version - the
 ...
 * v40 -> v41 (the guided tutorial): two additive schema
 * changes. `GameStateSchema` gained `tutorialStatus`
 ...
 * v41 -> v42 (the energy bar): `GameState.laborSlotsSpentToday`
 ...
 * v42 -> v43 (the tutorial actually guides): `GameStateSchema`
 ...
 * the field.
 * v43 -> v44 (the live room): the overnight auction bidder is
...
/**
 * v8 -> v9: reconstructs the real, index-addressable
...
/**
 * v11 -> v12 (auction rework II): reconstructs each active lot's
...
/**
 * v13 -> v14 (hidden issues): marks every existing `CarInstance`
...
/**
 * The v16 migration's old-group -> new-parts fan-out. A historical fact
...
/** v15 -> v16: `PartInstance.conditionPercent` -> `band`. Leaves
...
/**
 * v15 -> v16: reconstructs one `CarInstance`'s 29-part `parts`
...
function migrateCarInstanceToBands(car: unknown): unknown {
  ...
  // - it's a legacy shape from before an even older refactor, deliberately
  // left unmigrated (see the v5 entry in the SAVE_VERSION doc comment). Leave
  // it untouched rather than fabricating a `parts` map from nothing: the final
  // `GameStateSchema.parse` below must still reject it.
...
/**
 * v17 -> v18: hardcoded historical table picking one
 ...
 * original deadline from - the old flat `SERVICE_JOB_DEADLINE_DAYS`
 * sim constant's own value. Cosmetic only: an in-flight job's real
 * `dueOnDay` is preserved untouched either way, so this never actually
 * changes when the job is due. */
...
function migrateServiceJobToTasks(serviceJob: unknown): unknown {
  ...
  // This migration's output must conform to the CURRENT task shape, since no
  // later migration reshapes `tasks` again. `action`/`targetBand`/`minGrade`
  // collapse into one `requirement` (repair->slotCondition,
  // install->slotCondition-with-minGrade-and-minBand-'fine').
  const task =
    kind === 'install'
      ? {
          requirement: {
...
/**
 * v19 -> v20 (listings removed): resolves every genuinely pending
...
/**
 * v20 -> v21: one `grade: 'stock'` catalog part id per
...
/**
 * v22 -> v23: builds `toolTiers` from the save's
...
/**
 * v27 -> v28: the sibling SKU at `targetClass`, same (carPartId,
 ...
 * still active (the customer-job tag) - remapped to THAT job's car's
 ...
 * closed, is left alone - the "else common" default, which it
