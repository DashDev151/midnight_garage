...
follows one of two roof recipes to keep that read consistent across sixteen
...
usual 640x360 stage: this map holds sixteen locations plus a dense centre,
...
written for (sixteen locations across 960x540 rather than one 640x360
...
- `buildings.ts` - one indexed template per location, a colour map per
  ...
  and measure it. `OVERWORLD_LOCATION_IDS` lists all sixteen ids;
  `OVERWORLD_LOCATION_LABELS` is the display name shown when a location is
  hovered; `INERT_LOCATIONS` flags the two that are permanently drawn but go
  nowhere (the bank and the dealer network). The stand is a third shut
  building, but conditionally rather than permanently, so it is not on that
  list - see `overworldNav.ts`'s own `standUnlocked` flag.
...
960x540 composition holds together as a whole rather than as sixteen
