...
/** One staff member or job-ad candidate card,
...
/** A job-ad card - a candidate plus its bio, the
...
/** The live crew skill at the bench right now - the leading benched
...
/** The whole Staff Office view - the roster and the ads
...
export interface StaffOfficeView {
  ...
  maxStaff: number
  atCap: boolean
  /** The crew skill currently in effect at the bench, or null when
   * the bench is empty. */
...
/**
 * The Staff Office domain store. The staff surface (the
 ...
 * reads and writes it through `gameStore` - no second source of truth.
 */
export const useStaffStore = defineStore('staff', () => {
  const game = useGameStore()

  /**
   * The Staff Office's two panels - the current roster
   ...
   * posted/expiry days the card shows, plus the bench crew summary.
   */
  const staffOfficeView = computed<StaffOfficeView>(() => {
    ...
    // The crew skill in effect at the bench right now - the leading benched
    // skill per area (`crewSkillFor` reads the map: chassis leads the
  ...
  /** Hire the ad candidate - instant, the
  ...
  /** Dismiss a member - instant, no severance. The
  ...
  /** Schedule a member's bench/contract reassignment.
