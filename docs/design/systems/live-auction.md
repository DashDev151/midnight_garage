...
**Status:** LIVE. Sprint 110 promoted the prototype into the main game: every lot now
resolves in the room (`packages/game/src/screens/auctionRoom.ts` + `AuctionRoomScreen.vue`),
driven by `economy.json`'s `auctionRoom` block through `ROOM_TUNING` in
`packages/content/src/economy.ts`, and the overnight auction resolution it replaced is gone.
The dev-only demo (`auctionRoomDemo.ts` + `AuctionRoomDemoScreen.vue`) survives as the tuning
bench, running the same machine and the same config. This documents the model and, per the
maintainer's request, the clearing-price odds, as they stood at Sprint 105; Sprint 107's room
reactions and multi-rung bid sizing ship in the code and are not written up here.
