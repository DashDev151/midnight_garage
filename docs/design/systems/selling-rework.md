...
**Status: APPROVED by the maintainer (2026-07-22), including the venue-name pools, and
IMPLEMENTED by Sprint 114** (channels, the buyer read, the matched-sale bonus, and the venue
pools in `packages/content/src/venueNames.ts` and `buyer.ts`; section 3's receipts stay parked
by this doc's own call). The standing rule for any further tunable here: the sprint doc must
list every new tunable (channel fees, offer profiles, the matched-sale bonus value) for
explicit sign-off per CLAUDE.md directive 22 before any agent launches. The maintainer's
brief: selling is too flat; haggling is ruled out as thematically wrong; every channel a cost
and a purpose, EXCEPT the free-but-bad shop-front channel (a sign in the window, the worst
buyers), which is deliberately the zero-cost floor
