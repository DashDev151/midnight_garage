...
### 4.1 Where these numbers come from: the car performance model

When this spec was written, every field above was an open sourcing question, and the honest answer
was "estimate it per car and tune until it feels right". That is no longer true. The car performance
model (`docs/design/car-performance/`) carries measured per-car figures for most of this contract,
validated against real driven times. **Drive mode reads them; it does not re-derive them, and it does
not carry a second set.**

| `CarSpec` field | Source in the performance model |
|---|---|
| `massKg`, `weightBalanceFront`, `cgHeightM`, `wheelbaseM` | the car's own spec, already the model's inputs |
| `tyreGripMu` | the measured mechanical grip coefficient, split out of the two-speed lateral-g pair |
| `downforceCoeff` | the downforce coefficient from the same split, so grip rises with speed by a measured amount rather than a guessed one |
| `brakeForceN` | derived from the measured braking coefficient, which is its own input and is NOT equal to lateral grip |
| `peakPowerHp`, `drivetrainEfficiency` | the solved effective wheel power, which reproduces the car's measured acceleration rather than assuming peak power at every engine speed |
| `tractionCapFactor` | the solved launch acceleration, measured per car rather than set by a drivetrain constant |
| `drivetrain`, `awdSplitFront` | the layout tag, unchanged |

**What the performance model cannot supply, and why.** It is a quasi-static point-mass sim: it has
no yaw, no slip angle, and no transient. So `corneringStiffnessFront`/`Rear`, `yawInertiaScale`,
`steeringLockRad` and the friction-circle blend stay drive mode's own parameters. That is the whole
of what this feature has to invent, which is a far smaller problem than the one section 4 originally
posed.

**The acceptance test this buys, and it is the strongest one available.** The performance model has
measured lap times on four real courses, and the drive mode can be driven round the same geometries.
A correctly parameterised drive model should reproduce those lap times. That turns "does it feel
right" (unfalsifiable) into "does a clean lap land inside a few per cent of the number the
economy sim already shows the player" (a number, checkable, and it also guarantees the two halves of
the game never disagree about which car is faster). Add it to section 12.
