> **ARCHIVED.** The proposed rebuild architecture for the car spec arc, with the audit of what the sim actually read from a spec sheet. Superseded by `docs/design/car-performance/README.md`. Kept as the historical record; do not read it as current.
...
`docs/design/car-performance/car-spec-book.html` are the ground truth. Every performance quantity the game shows or
