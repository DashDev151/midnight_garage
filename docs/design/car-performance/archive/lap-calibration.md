> **ARCHIVED.** The running record of the maintainer's driven Forza lap times and how they calibrated the prototype pace model. Superseded by `docs/design/car-performance/README.md`. Kept as the historical record; do not read it as current.
...
**Reproduce:** `node docs/design/car-performance/lapsim/lapsim-report.cjs` (the `PREDICT` block).
