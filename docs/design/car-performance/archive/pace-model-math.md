> **ARCHIVED.** The lap-time mathematics as the prototype ran them: every formula, variable and constant. Superseded by `docs/design/car-performance/README.md`. Kept as the historical record; do not read it as current.
...
`docs/design/car-performance/archive/pace-model-design.md`. SI units throughout unless stated. Deterministic: no randomness,
