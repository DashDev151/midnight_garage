...
| O8 | Workshop view artwork | THE BIG ONE - see section 4. Three backdrop drawings (body panel schematic, engine bay from above, underside on the lift) behind a fixed 32-region hit map, plus the part sprites that survive on the docked panel, the bench strip and the parts market | Region state swaps; open/missing reads as removed art, not a tint | PLACEHOLDER (`workshopViewSprites.ts` backdrops + `partSprites.ts`, both explicitly launch-blocking in TODO) | P1 |
...
## 4. The service diagram redesign (interaction BUILT, art outstanding)
...
**The interaction half of that shipped in Sprint 120 and this section is now an art brief against a
fixed target, not an open design.** Two of the questions the catalogue recorded here are answered by
what was built; the rest stand.

- **View layout: ANSWERED.** Three views, not one composite: a plan-view body panel schematic,
  the engine bay from above, and the underside as on a lift. The lift view was the diegetic
  answer and it is what shipped, pairing with the garage's two-post lift (W2).
- **Generic car vs per-body-family: ANSWERED for v1.0, generic.** One shared region map serves
  every car in the roster. Per-family silhouettes remain the expensive option and are parked as a
  tier-3 idea in `IDEAS.md`, out of v1.0 by definition. Art commissioned against this section
  should assume one drawing set, not one per silhouette.
- **Region mapping:** the hit map the art must land on is `workshopViewLayout.ts`, and it is
  three views, not one diagram: Body (a stylised plan-view panel schematic, 8 regions: the
  bonnet, roof, boot and two side zones, plus aero, seats and dash/gauges), Engine bay (10
  ...
  teardown depth by drawing, never by overlapping hit areas.
- **The state vocabulary the art must carry.** A part region: condition band, uncertain,
  missing, legitimately absent (the NA car's empty turbo slot), planned work, and the disabled
  read. A zone region: three severity layers (metal 0-3, surface 0-2, finish 0-3), panel off,
  planned work, and the inert "no readings" state for a car with no zone data. Severity keeps
  a channel other than colour (rows of filled pips today), and states should read as art: a
  missing gearbox is a HOLE with a hoist strap, not a red tile.
- **Interaction unchanged:** the views are a selection surface and nothing else; a click
  reports the zone or part pointed at and the docked action panel does the acting. The art is
  a backdrop layer sitting under every region and outside the pointer path
  (`workshopViewSprites.ts` rasterises the placeholder today), so finished art drops in
  without touching `WorkshopViews.vue`, the layout module, or any sim logic.
...
4. **D4 Service diagram shape: CLOSED.** Generic (one shared region map for the whole roster)
   and three views, decided by what Sprint 120 built. O8 is no longer blocked on a decision; it
   is blocked only on the drawings themselves.
