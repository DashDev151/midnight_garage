...
**One row is now closed. Struck 2026-08-21 by the repair refactor arc:**

| closed defect | how it closed |
| --- | --- |
| **A repair click that resumes an open job silently ignores its own target band.** A "to mint" click landing while the fine job was still open spent labour finishing FINE, because `findOrCreateJob` keyed on car+kind+component and a resume returned the open job unchanged. Measured on the day-6 playtest: chassis "to mint", 8 labour, car sold with chassis at fine | **closed by construction, sprint 225.** A repair job's identity now includes its KIND, so Service, Rebuild and Restore are three separate jobs on one part, each locked to its own target band and each resuming only into itself (`repairJobIdFor(target, kind, context)`, `packages/sim/src/repairJobs.ts`). Proved on this exact defect's shape, and on `chassis` at that, by `repairJobCards.test.ts`'s `job identity` describe: "a service job and a rebuild job on the same part are two distinct jobs that do not interfere" (`:356`) and "resuming the rebuild after the interleaved service still finishes to its own locked target band" (`:385`). The workbench repair surface the defect was found on is deleted (sprint 230). The one control still creating a `repair-zone` job, the body shop's `part-repair` on `chassis`, cannot reproduce it either: it offers one rung at a time off `nextRepairStep` and never lets the player name a target band, so there is no button whose word a resume could ignore |
