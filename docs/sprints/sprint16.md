...
1. **Equipment reputation ladder — first-pass, tied to the existing price ladder's own ordering** (not
   ...
   | `upholstery-bench` | ¥350,000 | `local` |
...
- [ ] `equipment.json`: set `minReputationTier` per decision 1.
- [ ] `facilities.ts`/`facilities.json`: `Facilities`' per-kind config gains a `minReputationTier:
  ReputationTier[]` array parallel to `bayPricesYen`, populated per decision 2.

### B. Sim (`packages/sim`)

- [ ] `equipment.ts`: extract the pure ownership-lookup helper (decision 5); `hasEquipmentFor` becomes
  a thin wrapper over it.
- [ ] `facilities.ts`: `nextBayPriceYen`/`applyBayPurchase` gain a reputation check (same silent-
  refusal shape as equipment) reading the new content array.
- [ ] `constants.ts`: `JOB_HINT_OFFER_CHANCE` (decision 4).
- [ ] `catalogs.ts`: the collector-network-only skip generalizes to a per-tier threshold lookup
  covering all 4 auction tiers (decision 3).
- [ ] `serviceJobs.ts`: `generateServiceJobOffers` gains `ownedEquipmentIds`/`equipment` params (via
...
- [ ] **These gates hit the bots hard, and that impact was unexamined.** Bots start at `unknown`
...
- [ ] **Open design question to settle at implementation time:** should any bot get minimal
...
- [ ] New `screens/UpgradesScreen.vue` — Facilities + Equipment sections moved here wholesale from
...
- [ ] `GarageScreen.vue`: Facilities/Equipment sections removed; a nav link/button to `/upgrades` added.
- [ ] `router/index.ts`: new `/upgrades` route, lazy-loaded, matching the existing route shape exactly.
- [ ] `gameStore.ts`: purchase-refusal paths (`buyEquipment`/`buyBay`) already return `false` on any
...
- [ ] Sim: equipment/facility purchase tests extended for the reputation gate (owned-but-insufficient-
...
- [ ] Content: `gameState.test.ts`/schema tests updated for the new `Facilities` field.
- [ ] Game: `UpgradesScreen.test.ts` covering render + purchase flow (mirroring existing
  `GarageScreen.test.ts` facility/equipment coverage, since it's the same functionality relocated).
- [ ] Golden masters re-pinned if the scripted `advanceDay.test.ts` career's fixture equipment/bay
