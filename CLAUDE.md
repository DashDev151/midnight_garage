...
**Current state:** Sprint 21 (value model: condition, parts and market pressure set the price) is implemented, all checks green (typecheck/lint/format/test 546/546/build), pending commit — see `docs/sprints/sprint21.md`'s Exit section for the real measured numbers. Sprint 20 (auction rework II: open bidding) and Sprints 00 through 19 (plus same-day follow-up fixes 19b and 19c) are committed (`git log` has the hashes; not duplicated here). Sprints 22 through 24 are designed (`docs/sprints/sprint22.md`-`sprint24.md`) as the rest of the foundational-economy arc. Full sprint-by-sprint implementation detail — goals, decisions, deviations found during implementation, test counts, each sprint's own Exit section — lives one file per sprint in `docs/sprints/sprintNN.md`. This file intentionally does not duplicate that detail; read the current sprint's doc (and the previous one, per the Sprint workflow above) before starting new work, not this paragraph.
