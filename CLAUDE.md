...
**Current state:** Sprints 00 through 24 are implemented and committed (`git log` has every hash), the last five being the foundational-economy arc - 20 (open-bidding auctions), 21 (car value model), 22 (hidden issues and inspection), 23 (progression pacing and rent), 24 (debt/polish + the session-log seed). The maintainer playtest happened 2026-07-11 (raw notes in `docs/playtest-notes-2026-07-11.md`) and found the core repair/replace loop unsound: too-coarse component granularity, authored numbers where derived ones belong (job payouts blind to part prices, issue fix costs blind to car value, static book value), a dead market (one-shot auction demand rolls, weekly batch dumps, guaranteed-sale listings), and raw ids leaking into UI copy. The triage produced the **Loop Rework arc, Sprints 25-31** (docs designed 2026-07-11, implementation not started): 25 triage/guardrails, 26 the banded 29-part model (five named condition bands; the hidden-defect/inspection system is paused and removed per maintainer decision), 27 transparent cost-weighted value (clean value minus restoration bill) + full pre-bid condition visibility, 28 drill-down UI + catalog expansion (rotary parts, NA forced-induction kits, underglow), 29 service-jobs framework v2, 30 living auctions, 31 walk-in-only selling. Also on 2026-07-11: the em-dash ban (directive 15) landed with a repo-wide purge. Full sprint-by-sprint detail lives one file per sprint in `docs/sprints/sprintNN.md`; read the current sprint's doc (and the previous one, per the Sprint workflow above) before starting new work, not this paragraph.
