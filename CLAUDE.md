...
**ONE CARVE-OUT (maintainer amendment 2026-07-30): any task that RETIRES, RENAMES or RESHAPES a schema field or an exported symbol runs `pnpm typecheck` before it reports.** This is not redundant with the hook, because the hook runs too late by design. Sprint 136 retired `statModifiers.reliability` and `PartsMarketScreen.vue` kept reading it for two sprints; typecheck is whole-program, compiles every `.vue` template, and caught it instantly the one time it ran. Nine commits had landed on narrow per-file test runs before anyone pushed. Typecheck alone is the cheapest stage of the gate, and this carve-out covers only field-shape changes: it does NOT license running lint, format, coverage or the full suite by hand.
...
**Amendment (2026-08-13, behaviour-first governance, maintainer directive):** the maintainer no longer ratifies raw numbers, ever. For any value change within the economy's existing design shape: Claude states the intended FELT BEHAVIOUR in the sprint doc (what the number means, what the player should experience), chooses the value itself, re-pins the guard test in the same change with that behaviour statement recorded in its header, and the maintainer validates by playtest. Presenting a bare constant for ratification ("please sign off 0.3") is forbidden. Maintainer approval is still required for changes to the design SHAPE: new mechanics, new sinks or faucets, law scope, anything that changes what a system is rather than how hard it bites. The two analysis rules above (never charge fixed overheads against a single play; never treat worst-case probes as typical crises) stay in force unchanged.
...
the same wing gains an FD four seconds a lap and costs a Wagon R time. The four physical dial
curves were reviewed in Sprint 142 and deliberately left PROVISIONAL: no driven measurement of a
worn car exists, so a new value would be as unmeasured as the current one.
...
- `midnight-garage-roster.csv` - **the single source of truth for the full 94-car roster**: one
  row per car, 58 columns keyed on a permanent `uid`, every per-car value the game has (identity, price, production window, tier, rarity,
