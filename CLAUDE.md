...
**Current state:** Sprint 20 (auction rework II: open bidding) is implemented, all checks green, and is being committed together with the 2026-07-10 review batch (files are entangled) — see `git log` for the hash once committed. Sprints 00 through 19 (plus same-day follow-up fixes 19b and 19c) are also committed (`git log` has the hashes; not duplicated here). Sprints 21 through 24 are designed (`docs/sprints/sprint21.md`-`sprint24.md`) as the rest of the foundational-economy arc. Full sprint-by-sprint implementation detail — goals, decisions, deviations found during implementation, test counts, each sprint's own Exit section — lives one file per sprint in `docs/sprints/sprintNN.md`. This file intentionally does not duplicate that detail; read the current sprint's doc (and the previous one, per the Sprint workflow above) before starting new work, not this paragraph.
